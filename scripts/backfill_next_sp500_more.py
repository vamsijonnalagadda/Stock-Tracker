#!/usr/bin/env python3
"""More-aggressive backfill for S&P500: try more earnings events and longer history.

Non-destructive: only fills fields when missing.
"""
import json
import time
...
def compute_moves_for_symbol(sym, max_events=12):
    t = yf.Ticker(sym)
    try:
...
def main():
    ...
    for i, sym in enumerate(targets, 1):
        ...
        res = compute_moves_for_symbol(sym, max_events=12)
        if res:
            for day_rows in days.values():
                for row in day_rows:
                    if str(row.get('symbol') or '').upper() == sym:
