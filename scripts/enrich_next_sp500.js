...
// Attempt to fetch the API; if it's not available (container run), fall back to local file.
(async function main(){
  try {
    let data = null;
    try {
      ...
      try {
        data = JSON.parse(fs.readFileSync(OUT_PATH, 'utf8'));
        console.log('Loaded next-week from local file');
      } catch (e2) {
        throw e;
      }
    }
    ...
    const need = rows.filter(r => r.isSp500 && (!r.companyName || String(r.companyName).trim().toUpperCase() === String(r.symbol||'').trim().toUpperCase()));
    const symbols = [...new Set(need.map(r=>String(r.symbol||'').toUpperCase()).filter(Boolean))];
    console.log('S&P symbols needing name enrichment:', symbols.length);
