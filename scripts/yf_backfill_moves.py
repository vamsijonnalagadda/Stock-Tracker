...
def compute_moves_for_symbol(sym, max_events=6):
    ...
    moves = []
    # edf index are timestamps; iterate most recent first
    for idx in list(edf.index)[:max_events]:
        try:
            ...
            continue
        # determine before-market vs after-market
        hour_code = 'amc' if (hour >= 16) else 'bmo'

        # find nearest trading date index
        if ev_date in date_list:
            i = date_list.index(ev_date)
        else:
            # first date greater than event
...
def main():
    ...
    rows = [r for day in ew.get('days', {}).values() for r in day]
    symbols = []
    for r in rows:
    ...
    for i, sym in enumerate(symbols, 1):
        ...
        if res:
            ...
            for day_rows in ew.get('days', {}).values():
                for row in day_rows:
